...
def d_mse_w(y, y_hat, x):    
    return np.matmul(x.T, y_hat - y)

def d_mse_w_0(y, y_hat):    
...
def train_gd(x, y, num_steps=1000, lr=0.01, regularization_coeff=0, method='linear'):
    _w = np.random.random(x.shape[1])   
    _b = np.random.random(1)

    _w_trail_path = [_w]
    _b_trail_path = [_b]
    
    for i in range(0, num_steps):
        _y_hat = globals()[method](x, _w, _b)
        
        _w -= lr * ((d_mse_w(y, _y_hat, x) / x.shape[0]) + (regularization_coeff/x.shape[0] * _w))
        _b -= lr * d_mse_w_0(y, _y_hat) / x.shape[0]
        
        _w_trail_path.append(lr * d_mse_w(y, _y_hat, x) / x.shape[0])
        _b_trail_path.append(lr * d_mse_w_0(y, _y_hat) / x.shape[0])
        
    return _w, _b, _w_trail_path, _b_trail_path

def cross_binary(y_hat, y):
    return np.sum(- y * np.log(y_hat) - (1-y) * np.log (1 - y_hat), axis=0)

def sigmoid(x):
    return 1/(1 + np.exp(-x))

def logistic(x, w, b, threshold=0.5):
    return np.where(sigmoid(linear(x, w, b)) >= threshold, 1, 0)
